...
Term::Term(SDL_Renderer *r, int x, int y, int w, int h) : renderer(r), cursor_x(0), cursor_y(0), screen_y(0)
{
...
void Term::newline()
{
	...
	if (cursor_y >= lines.size())
		add_line();
	if (cursor_y == screen_y + h_in_char)
...
{
	if (ch >= 128)
		printf("ch %d is not a valid char\n", ch);
	assert(ch < 128);
	if (is_cursor_on_screen())
		set_dirty_char(cursor_y - screen_y, cursor_x);
	int y = cursor_y;
...
void Term::draw_cursor()
{
	if (is_cursor_on_screen())
	{
		...
		rect.y += CH_HEIGHT * y;
		rect.x += CH_WIDTH * x;
		SDL_Texture *t = is_cursor_visible ? (is_focus ? get_focus_cursor_texture : cursor_texture) : ch2texture_term(' ');
